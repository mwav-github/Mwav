...
import net.mwav.common.module.MwavFactory;
...
public class SampleController {
	 ...
	 * 
	 * @Resource어노테이션을 통해서 필요한 빈(bean)을 수동으로 등록하는것이다. 그리고 수동으로 등록할 빈의 이름이
	 * "sampleService"이고, 이는 @Service("sampleService")라고 선언했을 때의 그 이름인것을 확인한다.
	 * 
	...
	@Resource
	private MwavFactory mavFactory;

	/*
	 * @RequestMapping 이라는 부분이 웹 클라이언트 (jsp)에서 들어온 요청에 해당하는 비지니스 로직을 찾아주는 역할을 한다. 뒤에
	 * method는 이 요청이 POST 인지, GET 방식인지를 말해주는데 우리는 앞으로 거의 대부분을 POST로 보낼것이기 때문에, 이 부분은
	 * 지울 계획이다.
	 *
	 * @RequestMapping(value = "/", method = RequestMethod.GET) //어노테이션 설정
	 * /sample/openBoardList.mwav으로 url요청이 들어오면 이 주소는 @RequestMapping 어노테이션과 매핑되어,
	 * 해당 메서드가 실행된다. ModelAndView 객체를 생성하게 된다. 페이지와 매핑된다 실제 디렉토리 구조 + .jsp.
	 */
	...
	public ModelAndView openBoardList(CommandMap commandMap) throws Exception {
		...
		/*
		 * list라는 단순한 변수의 선언 각각 키와값으로 구성 sampleService.selectBoardList 메서드를 통해 얻어온 결과
