...
public class CommonServiceImpl implements CommonService {
	...
	public Map<String, Object> insertGdsUpLoader(Map<String, Object> map,
		...
		/* 파일업로드
		 * 
		 *  http://toriworks.tistory.com/entry/Spring%EC%97%90%EC%84%9C-%ED%8C%8C%EC%9D%BC-%EC%97%85%EB%A1%9C%EB%93%9C-%EA%B5%AC%ED%98%84%ED%95%B4-%EB%B3%B4%EA%B8%B0
		 *  */
		// /////////////////////////
		...
		/*
		 * (MultipartHttpServletRequest)request; 부분이다. 위에서 첨부파일은 Multipart 형식의
		 * 데이터이며, HttpServletRequest에 담겨서 서버로 전송된다고 이야기하였다. HttpServletRequest 그
		 * 자체로는 Multipart형식의 데이터를 조작하는데 어려움이 있기 때문에, MultipartHttpServletRequset
		 * 형식으로 형변환을 하였다.
		 */
		 ...
		 * 
		 * Map에서 데이터를 가져오려면 그 Map에 있는 키(Key)를 알아야 접근이 가능하다거나, for문 등의 반복문을 사용할수가
		 * 없는것을 생각하면 된다. 여기서는 Iterator를 이용하여 Map에 있는 데이터를 while문을 이용하여 순차적으로
		 * 접근하려고 한다.
		 */
		...
		while (iterator.hasNext()) {
			multipartFile = multipartHttpServletRequest
					.getFile(iterator.next());
			/*
			 * MultipartFile 객체에 request에서 파일 객체를 가져오는 부분이다.
			 * multipartHttpServletRequest의 getFile() 메서드는 request에 저장된 파일의
			 * name을 인자로 받는다. *.
			 * 
			 * 이 name을 Iterator를 통해서 가져온다고 이야기를 하였다. 그것이 Iterator.next() 메서드다.
			 * 
			 * hasNext() 메서드는 Iterator 내에 그 다음 데이터의 존재 유무를 알려주고, next() 메서드는
	...
	public GeneralConfig getFrontFooter(HttpServletRequest request, String xml) throws Exception {
		...
		watchMap.put("cacheKey", xml);
		
		WatchController watch = new WatchController();
		watch.watchService(watchMap);
		
		System.out.println("서비스 Cacheable");
		
		GeneralConfig generalConfig = (GeneralConfig) XmlLib.getInstance().unmarshal(filePath+fileName, GeneralConfig.class);		
