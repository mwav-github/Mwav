...
public class MemberController {
	...
	// 0번
	@RequestMapping(value = "/memberDefault.do") // ROOT/ /memberDefault.do 경로로 오는 URL은 모두 이 Controller에 매핑
	//RequestMapping을 이용한 URL mapping. / memberService 다음에 /memberDefault가 오면 아래 메서드로 매핑된다.
	...
	public ModelAndView selectLogin(CommandMap commandMap,
		...
		if (memberLogin != null && loginCheck != 5) {
			...
			if (b_mbrLoginPw.equals(a_mbrLoginPw)) {
				// login 성공
				loginCheck = 1;
	...
	// 2번 추후 return 필요없을듯 고민
	@RequestMapping(value = "/member/memberForm.do")
	public ModelAndView insertMemberForm(CommandMap commandMap,
		...
		memberService.insertMemberForm(commandMap.getMap());
	...
	public ModelAndView selectMemberView(CommandMap commandMap,
			HttpServletRequest request) throws Exception {
		HttpSession session = request.getSession();
		...
		Map<String, Object> selectMemberView = memberService
				.selectMemberView(commandMap.getMap());

		if (selectMemberView != null && !selectMemberView.isEmpty()) {
			System.out.println("view 줄랭");
			mv.addObject("mode", "SDMyPage");
			mv.addObject("selectMemberView", selectMemberView);
		}
		return mv;
	}
	...
	public void selectIdCheck(HttpServletRequest request,
			HttpServletResponse response) throws Exception {

		String mbrLoginId = request.getParameter("mbrLoginId");

		boolean selectIdCheck = memberService.selectIdCheck(mbrLoginId);
	...
	public ModelAndView updateMemberDelete(CommandMap commandMap,
			HttpServletRequest request) throws Exception {
		ModelAndView mv = new ModelAndView("/Index");
		...
		boolean updateMemberDelete = memberService
				.updateMemberDelete(commandMap.getMap());

		mv.addObject("updateMemberDelete", updateMemberDelete);
	...
	// 6번 추후
	@RequestMapping(value = "/member/IdFinder.do")
	public void selectIdFinder(CommandMap commandMap,
			HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		String selectIdFinder = memberService.selectIdFinder(commandMap
				.getMap());
	...
	// 7번 추후
	@RequestMapping(value = "/member/PWFinder.do")
	public @ResponseBody boolean selectPWFinder(CommandMap commandMap,
			HttpServletRequest request) throws Exception {

		boolean selectPWFinder = memberService.selectPWFinder(commandMap
				.getMap());

		if (selectPWFinder == true) {
			// 찾았다.
		...
		return selectPWFinder;

	}

	// 7번 추후
	@RequestMapping(value = "/member/imsiPWFinder.do")
	public @ResponseBody boolean selectimsiPWFinder(CommandMap commandMap,
			HttpServletRequest request) throws Exception {

		boolean selectimsiPWFinder = memberService
				.selectimsiPWFinder(commandMap.getMap());

		return selectimsiPWFinder;
	...
	// 4번 추후
	@RequestMapping(value = "/member/PWUpdate.do")
	public ModelAndView updatePw(CommandMap commandMap,
			HttpServletRequest request) throws Exception {
		...
		boolean updatePw = memberService.updatePw(commandMap.getMap());

		mv.addObject("updatePw", updatePw);
	...
	public List<Map<String, Object>> selectpostList_2(
        ...
        // 만약 특정 DTO(VO)의 형태로 자동 매핑하여 받을 경우, 해당 타입으로 정의 (ex: @RequestBody User user)
		...
		//Service에서 리턴된 값을 받는 부분. 주로 DB처리 후의 값을 Service단(jsonControllerService.selectDashboardList)에서 해줌
