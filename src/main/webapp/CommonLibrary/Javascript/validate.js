...
*/	
	...
	/*  if (mbrLoginPW.length < 8 || mbrLoginPW.length > 255) {
  alert("비밀번호는 8 ~ 255 자리로 입력해주세요.");
  return false;
 } */
...
function Validate() {
	...
	//첫글자는 반드시 알파벳 이면 true
	Validate.prototype.isOnlyFirstAlpabetic = function(str) {
		var invalidCharactersRegExp = /^[a-zA-Z]/g;
	...
	// 숫자 + '-' true
	Validate.prototype.isNumeric = function(str) {
		var invalidCharactersRegExp = /[^\d\-]/;
		var isValid = !(invalidCharactersRegExp.test(str));
		return isValid;
	}
	...
	// 임시 사업자번호체크
	Validate.prototype.isCheckImsiBusRegNo = function(str) {
		var invalidCharactersRegExp = /D[0-9]{9}/;
		var isValid = (invalidCharactersRegExp.test(str));
		return isValid;
	}
	...
	Validate.prototype.isOnlyDate = function(yyyy, mm, dd) {
		...
		var enteredDate = new Date(dd + " " + Validate.prototype.MMtoMon(mm)
				+ " " + yyyy);
		if (enteredDate.getDate() != dd) {
			isValid = false;
		}
		return isValid;
	}

	Validate.prototype.isOnlyDate2 = function(yyyymmdd) {
		var isValid = true;
		var enteredDate = new Date(yyyymmdd.substring(6, 8) + " "
				+ Validate.prototype.MMtoMon(yyyymmdd.substring(4, 6)) + " "
				+ yyyymmdd.substring(0, 4));
		if (enteredDate.getDate() != yyyymmdd.substring(6, 8)) {
			isValid = false;
		}
		return isValid;
	}
	...
	Validate.prototype.isOnlyDateYMD = function(ymd) {
		var yyyy, mm, dd
		if (ymd.length != 8) {
			return false;
		} // length : year(4), month(2), day(2)
		if (!this.isOnlyNumeric(ymd)) {
			return false;
		} // check number
		if (ymd.substring(0, 2) != "19" && ymd.substring(0, 2) != "20") {
			return false;
		} // 1899 < year < 2100
		yyyy = parseInt(ymd.substring(0, 4)); // year string -> integer
		if (ymd.charAt(4) == "0")
			mm = parseInt(ymd.charAt(5));
		else if (ymd.charAt(4) == "1")
			mm = parseInt(ymd.substring(4, 6))
		else
			return false; // month string -> integer
		if (ymd.charAt(6) == "0")
			dd = parseInt(ymd.charAt(7))
		else if (ymd.charAt(6) == "1" || ymd.charAt(6) == "2"
				|| ymd.charAt(6) == "3")
			dd = parseInt(ymd.substring(6, 8))
		else
			return false; // day string -> integer
		if (mm == 0 || dd == 0 || mm > 12 || dd > 31 || (mm == 2 && dd > 29)
				|| (mm == 2 && yyyy % 4 != 0 && dd > 28))
			return false;
	...
	Validate.prototype.isOnlyDateBirth = function(ymd) {
		var birth = new Date(ymd.substring(0, 4), ymd.substring(4, 6) - 1, ymd
				.substring(6, 8));
		var now = new Date();
		if (!this.isOnlyDateYMD(ymd)) {
			return false;
		}
		if (birth > now) {
			return false;
	...
	Validate.prototype.isValidPhone = function(phone_number) {
		if (!this.isOnlyNumeric(phone_number))
			return false;
		var regExp_phoneNum = /^(15.{2}|16.{2}|18.{2}|050.{1}|070|02|031|032|033|041|042|043|044|051|052|053|054|055|061|062|063|064|010|011|016|017|018|019)\d{4,8}$/;
		// var regExp_phoneNum =
		// /^(070|050|02|031|032|033|041|042|043|051|052|053|054|055|061|062|063|064|010|011|016|017|018|019)-?\d{3,4}-?\d{4}$/;
		var isValid = regExp_phoneNum.test(phone_number);
		return isValid;
	}

	// 휴대폰 및 지역번호만 true
	Validate.prototype.isValidCellPhone = function(cellphone) {
		if (!this.isOnlyNumeric(cellphone))
			return false;
		var regExp_cpNum = /^(070|02|031|032|033|041|042|043|044|051|052|053|054|055|061|062|063|064|010|011|016|017|018|019)\d{7,8}$/;
		var isValid = regExp_cpNum.test(cellphone);
		return isValid;
	}
	...
	// 년이 맞으면 true
	Validate.prototype.isValidYYYY = function(yyyy) {
		var isValid = true;
		var enteredDate = new Date("01 Jan " + yyyy);
		if (enteredDate.getDate() != '01') {
			isValid = false;
		}
		return isValid;
	}
	...
	// 월이 맞으면 true
	Validate.prototype.isValidMM = function(mm) {
		var invalidCharactersRegExp = /01|02|03|04|05|06|07|08|09|10|11|12/;
		var isValid = invalidCharactersRegExp.test(mm);
		return isValid;
	}
	...
	// 길이가 최대값을 넘으면 false, 아니면 true
	Validate.prototype.chkMaxLength = function(obj, minlen, maxlen) {
		return (obj.value.length < min && obj.value.length > maxlen) ? false : true;
	}

	// 크기(byte)가 최대값을 넘으면 false, 아니면 true
	...
	// 2008-11-21 안병호 추가
	// 숫자체크 0~9 ←→↑↓ del, backSpace 등 기본키 허용
	// Validate.prototype.fc_onfocus 에서 호출
	Validate.prototype.fc_onlyNum_check = function() {
		var key = event.keyCode;
		if (!(key == 8 || key == 9 || key == 13 || key == 46 || key == 144
				|| (key >= 96 && key <= 105) || (key >= 48 && key <= 57)
				|| key == 110 || key == 190 || (key >= 37 && key <= 40))) {
			// alert('숫자만 입력 가능합니다');
			event.returnValue = false;
		}
	}
	...
	Validate.prototype.checkScriptTag = function(obj, txt) {
		if (!validator.isAllSpace(obj.value)) {
			var regExp = /<script/;
			var str = obj.value.replace(/[^a-zA-Z<\/>]|[\s]+/g, "");
			str = str.toLowerCase(str);
			if (regExp.test(str)) {
				alert(txt + "에 script 태그는 사용할 수 없습니다.    ");
				obj.focus();
				return true;
			} else {
				return false;
 ...
 * 4~20글자만 가능합니다.")) return false;
 *  // /[\{\}\[\]\/?.;:|\)*~`!^\+<>@\#$%&\\\=\(\'\"]/gi 특수문자 포함 var
 * special_pattern = /[`~!@#$%^&*|\\\'\";:\/?]/gi; if (chk(special_pattern,
 * mbrLoginId, "특수문자는 사용 불가능합니다.")) return false;
 *  // /[\s]/g; 공백포함 포함 불가 var blank_pattern = /[\s]/g; //공백포함 불가 ==> true 나와야
 * return 하니까 // 공백 포함안된것이 true로 나와야 ! 맞다. if (chk(blank_pattern, mbrLoginId,
 * "공백은 사용 불가능합니다.")) return false;
 *  // 아이디 검사 // 첫 글자는 반드시 영문 소문자, 4~12자로 이루어지고, 숫자가 // 하나 이상 포함되어야 한다. 영문소문자와
 * 숫자로만 이루어져야한다. // \d : [0-9]와 같다. {n,m} : n에서 m까지 글자수 if
 * (!chk(/^[a-z][a-z\d]{3,11}$/, mbrLoginId, "첫글자는 영문 소문자, 4~12자 입력할것!")) return
 * false;
 *  // 이메일 검사 // 4글자 이상(\w = [a-zA-Z0-9_], [\w-\.]) @가 나오고 // 1글자 이상(주소). 글자 가
 * 1~3번 반복됨 if (!chk(/^[\w]{4,}@[\w]+(\.[\w-]+){1,3}$/, mbrEmail, "이메일 형식에
 * 어긋납니다.")) return false;
 * 
 * 
 * if (!chk(/[0-9]/, mbrLoginId, "숫자 하나이상포함!")) return false;
 *  // 비밀번호 확인 검사 if(pass.value!=pass1.value) { alert("비밀번호 확인!"); } var
 * Pass_pattern = /^(?=.*[a-zA-Z])(?=.*[!@#$%^*+=-])(?=.*[0-9]).{8,255}$/; if
 * (!chk(Pass_pattern, mbrLoginPw, "패스워드를 정확히 입력해주세요.")) return false;
 * 
 * if (!chk(blank_pattern, mbrLoginId, "공백은 사용 불가능합니다.")) return false;
 * 
 * if(!chk(/[0-9]/, id, "숫자 하나이상포함!")) return false;
 *  // 이름 검사 // 2글자 이상, 한글만 // 통과하지 못하면 한글로 2글자 이상을 넣으세요~ alert 출력!
 * if(!/^[가-힝]{2,}$/, name, "한글로 2글자 이상을 넣으세요~") return false;
 *  // 전화번호 검사 // 전화번호 앞자리는 2~3자리 숫자, 두번째 자리는 3~4자리 숫자 // 세번째 자리는 4자리 숫자 if
 * (member_CellularP_1.value != '') { if (!chk(/^0(2|1[01679])$/,
 * member_CellularP_1, "번호 2자리 이상 입력")) return false; if (!chk(/^[0-9]{3,}$/,
 * member_CellularP_2, "번호 3자리 이상 입력")) return false; if (!chk(/^[0-9]{4}$/,
 * member_CellularP_3, "4자리 번호 입력")) return false; }
 *  }
 * 
