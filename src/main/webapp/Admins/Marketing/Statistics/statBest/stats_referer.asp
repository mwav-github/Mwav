...
sSQL="SELECT IP_Came_From, Count(IP_Came_From) as Hits"
sSQL = sSQL & " FROM tbl_Log_Visits"
sSQL = sSQL & " WHERE (not isnull(IP_Came_From) AND IP_Came_From<>'')"
sSQL = sSQL & " GROUP BY IP_Came_From "
'sSQL = sSQL & "HAVING hitDate between #" & DateFrom & "# and #" & DateTo & "# "
sSQL = sSQL & "ORDER BY Count(IP_Came_From) DESC, IP_Came_From"
...
ELSE
	...
	RESPONSE.WRITE "<TR><TH colspan=" & TableWidth+1 & " BGCOLOR='mediumslateblue'><font color='white'>Who Has Referred Most To This Site</TH></TR>"
	...
	For CurrentRow=1 to NumberRows
		...
		RESPONSE.write "<TR>"
		RESPONSE.write "<TD bgcolor='darkblue'><FONT COLOR='white'>" & rsLog("IP_Came_From") & "</font></TD>"
		RowData=rsLog("Hits")
		For Column=1 to TableWidth
