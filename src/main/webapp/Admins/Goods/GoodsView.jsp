...
<script src="/CommonApps/Goods/goodsView.js"></script>
	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-md-9">
				<!-- 소제목 -->
				<div class="col-lg-12">
					<h2 class="page-header">Goods View</h2>
				</div>
									...
									<div class="panel-body">
										<div class="row">
											<div class="col-sm-12 col-md-4 col-lg-4 " align="center">
															...
															<div class="col-sm-12" id="carousel-bounding-box">
																<div class="carousel slide" id="myCarousel">
																	...
																	<div class="carousel-inner">
																		<c:choose>
																			<c:when test="${fn:length(goodsFiles) > 0}">
																				<!-- http://fruitdev.tistory.com/132 -->
																				<%--http://marobiana.tistory.com/9 --%>
																				<c:forEach var="lsFile" items="${goodsFiles}" varStatus="status">
																					...
																					<div ${status.first ? 'class="active item"' : 'class="item"'} data-slide-number="${status.index}">
																						<a href="#" class="thumbnail" data-toggle="modal" data-target="#lightbox"> <img src="/xUpload/GoodsData/Goods-${selectOneGdsView.goods_id}/${lsFile.gFileName}">
																						</a>
																					</div>
																			...
																			<c:otherwise>
																				<div class="active item" data-slide-number="0">
																					<a href="#" class="thumbnail" data-toggle="modal" data-target="#lightbox"><img src="http://placehold.it/770x300&text=one"></a>
																				</div>
																			</c:otherwise>
																		</c:choose>
																	</div>
														...
														<c:when test="${fn:length(goodsFiles) > 0}">
															...
															<c:forEach var="lsFile" items="${goodsFiles}" varStatus="status">
																...
																<ul class="hide-bullets">
																	...
																	<li class="col-sm-6"><a class="thumbnail" id="carousel-selector-${status.index}"> <img src="/xUpload/GoodsData/Goods-${selectOneGdsView.goods_id}/${lsFile.gFileName}">
																	</a></li>
																</ul>
															</c:forEach>
														</c:when>
												...
												<jsp:include page="/Admins/Goods/LargeImageView.jsp" flush="false"></jsp:include>
											</div>
												...
												<table class="table table-user-information">
													...
													<tbody>
														...
														<tr>
															...
															<td colspan="3">${selectOneGdsView.goods_id }</td>
														</tr>
														...
														<tr>
															...
															<td colspan="3">${selectOneGdsView.gModelNbr}</td>
														</tr>

														<tr>
															<td class="info">상품명</td>
															<td colspan="3">${selectOneGdsView.gName}</td>
														</tr>

														<tr>
															<td class="info">셋트상품여부</td>
															<td colspan="3">${selectOneGdsView.isGoodsSet=='1' ? '예' : '아니오'}</td>
														</tr>

														<%-- 한 행에 따른 대는 td 두개인데 여기만 4개 이렇게 할 수가 없다 그러면 틀어지기 때문에 아래와 같이 임시방편으로.. --%>
														<tr>
															<td class="info">일반판매가</td>
															<td>${selectOneGdsView.gConsumerPrice}</td>
															<td class="info">회원판매가</td>
															<td>${selectOneGdsView.gMemberPrice}</td>
														</tr>

														<tr>
															<td class="info">제조(개발)사</td>
															<td colspan="3">${selectOneGdsView.gManufacturer}</td>
														</tr>
														<tr>
															<td class="info">원산지</td>
															<td colspan="3">${selectOneGdsView.gPlaceofOrigin}</td>
														</tr>

														<c:set var="gWhoDeliveryCost" value="${selectOneGdsView.gWhoDeliveryCost}" />
														<tr>
															<td class="info">배송비 지불 주체</td>
															<td colspan="3">
																<c:if test="${gWhoDeliveryCost=='C'}" >고객</c:if>
																<c:if test="${gWhoDeliveryCost=='P'}" >파트너사, 제조사</c:if>
																<c:if test="${gWhoDeliveryCost=='S'}" >자사</c:if>
															</td>
														</tr>

														<tr>
															<td class="info">배송비</td>
															<td>${selectOneGdsView.gDeliveryCost}</td>
															<td class="info">상품상태</td>															
															<td><c:if test="${selectOneGdsView.gStatus=='1'}" >판매가능</c:if></td>
														</tr>

														<tr>
															<td class="info">적립금</td>
															<td>${selectOneGdsView.gRsvFund}</td>
															<td class="info">포인트</td>
															<td>${selectOneGdsView.gPoint}</td>
														</tr>

														<tr>
															<td class="info">키워드</td>
															<td colspan="3">${selectOneGdsView.gKeywords}</td>
														</tr>

														<tr>
															<td class="info">성과급률</td>
															<td colspan="3">${selectOneGdsView.gFruitRate}</td>
														</tr>

														<tr>
															<td class="info">정렬순서</td>
															<td colspan="3">${selectOneGdsView.gOrder}</td>
														</tr>

														<tr>
															<td class="info">게시기간</td>
															<td>
																<fmt:formatDate value="${selectOneGdsView.gPostStart}" pattern="yyyy.MM.dd" />
															</td>
															<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; ~ &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
															<td>
																<fmt:formatDate value="${selectOneGdsView.gPostEnd}" pattern="yyyy.MM.dd" />
															</td>
														</tr>
													</tbody>
												</table>
											...
											<div class="col-md-12">
												...
												<textarea class="form-control" id="exampleFormControlTextarea1" rows="3" readonly>${selectOneGdsView.gNote}</textarea>
											</div>
										</div>
