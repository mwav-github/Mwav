...
<% if Session("MbrID") = "" then %>
	...
	if (document.frm.userEmail.value.length > 1 )  {

	str = document.frm.userEmail.value;
	...
	if (document.frm.replyEmail == "object" && document.frm.replyEmail.checked == true) {
	...
	var encrypted = '';
	var it = document.frm.userPW.value;
	var b = '0';var chars = it.split("");while(b<it.length){c = '0';while(c<letters.length){if(split[c] == chars[b]){if(c == "0") { c = ""; }if(eval(c+10) >= letters.length){bNbr = eval(10-(letters.length-c));encrypted += split[bNbr];}else{bNbr = eval(c+10);encrypted += split[bNbr];}}c++;}b++;}document.frm.userPW.value = encrypted;encrypted = '';
	*/
...
function ShowProgress() {
   ...
   if (document.frm.allfile1.value != ""<% if upFormCnt > 1 then %> || document.frm.allfile2.value != ""<% end if %><% if upFormCnt > 2 then %> || document.frm.allfile3.value != ""<% end if %><% if upFormCnt > 3 then %> || document.frm.allfile4.value != ""<% end if %>) {
...
function DoUpload() {
  ...
  document.frm.action = "abc_form_ok.asp?ID=" + theUniqueID;
}
...
<body bgcolor="<%=tb_bgcolor%>" marginwidth="0" marginheight="0" leftmargin="0" topmargin="0" onload="document.frm.<% if Session("MbrID") <> "" then %>bTitle<% else %>userName<% end if %>.focus();">
...
<form name="frm" method="POST" action="<% if bType > 0 then %><%=uploadType%>_<% end if %>form_ok.asp"<% if bType > 0 then %> enctype="MULTIPART/FORM-DATA"<% end if %>>
<tr>
...
<tr bgcolor="#F7F7F7" height="25">
	...
	<td width="460" colspan="3" class="font1">
		<input type=file name="allfile<%= i %>" size="30" class="form_input"> &nbsp; &nbsp; <%=maxsize%>MB 까지 가능
	</td>
</tr>
<%
...
%>
<tr>
	<td colspan="4" class="font1" bgcolor="#F7F7F7" style="padding-left:30px;color:#999999;"><%=fName%> 이 등록되어있습니다.
...
%>
		<!--input type="hidden" name="oldfilename<%= i+1 %>" value="<%= fName %>">
		<input type="hidden" name="oldfilesize<%= i+1 %>" value="<%=fSize%>"-->
<%
		next
