...
//  email :	sai_freelance@yahoo.com
	...
	var imgDir = "/CommonApps/Calendar/DateSelector/Images/"	// directory for images ... e.g. var imgDir="/img/"

	var gotoString = "현재 월로 이동합니다."//Go To Current Month
	var todayString = "오늘은" //Today is
	var weekString = "Wk"
	var scrollLeftMessage = "클릭하면 이전달로 이동합니다. 버튼를 누르고 있으면 자동스크롤 됩니다."//Click to scroll to previous month. Hold mouse button to scroll automatically.
	var scrollRightMessage = "글릭하면 다음달로 이동합니다. 버튼를 누르고 있으면 자동스크롤 됩니다."//Click to scroll to next month. Hold mouse button to scroll automatically.
	var selectMonthMessage = "월을 선택하세요."//Click to select a month.
	var selectYearMessage = "년도를 선택하세요."//Click to select a year.
	var selectDateMessage = "선택날자는 [date] 입니다."//Select [date] as date." // do not replace [date], it will be replaced by date.
    ...
    */
	...
	{
		//alert("dateFormat = " + dateFormat); mm/dd/yyyy
	...
	function constructMonth() {
		...
		if (!monthConstructed) {
			...
			for	(i=0; i<12;	i++) {
				...
				sHTML += "<tr><td id='m" + i + "' onmouseover='this.className=\"dropdown-select-style\"' onmouseout='this.className=\"dropdown-normal-style\"' onclick='monthConstructed=false;monthSelected=" + i + ";constructCalendar();popDownMonth();event.cancelBubble=true'>&nbsp;<font color='#ffffff'>" + sName + "</font>&nbsp;</td></tr>"
			}
	...
	function incYear() {
		for	(i=0; i<7; i++){
			...
			{ txtYear =	"&nbsp;<B><font color='#ffffff'>"	+ newYear +	"</font></B>&nbsp;" }
			else
	...
	function constructYear() {
		...
		if (!yearConstructed) {
			...
			sHTML =	"<tr><td align='center'	onmouseover='this.className=\"dropdown-select-style\"' onmouseout='clearInterval(intervalID1);this.className=\"dropdown-normal-style\"' onmousedown='clearInterval(intervalID1);intervalID1=setInterval(\"decYear()\",30)' onmouseup='clearInterval(intervalID1)'><font color='#ffffff'>-</font></td></tr>"
			j =	0
			...
			for	(i=(yearSelected-3); i<=(yearSelected+3); i++) {
				...
				sHTML += "<tr><td id='y" + j + "' onmouseover='this.className=\"dropdown-select-style\"' onmouseout='this.className=\"dropdown-normal-style\"' onclick='selectYear("+j+");event.cancelBubble=true'>&nbsp;<font color='#ffffff'>" + sName + "</font>&nbsp;</td></tr>"
				j ++;
			}

			sHTML += "<tr><td align='center' onmouseover='this.className=\"dropdown-select-style\"' onmouseout='clearInterval(intervalID2);this.className=\"dropdown-normal-style\"' onmousedown='clearInterval(intervalID2);intervalID2=setInterval(\"incYear()\",30)'	onmouseup='clearInterval(intervalID2)'><font color='#ffffff'>+</font></td></tr>"
	...
	function constructCalendar () {
		...
		sHTML =	"<table	border=0 class='body-style' height='150' border='0'><tr>"
