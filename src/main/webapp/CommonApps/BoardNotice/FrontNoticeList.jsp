...
<div class="bs-component">
	...
	<table class="table table-striped">
		...
		<%--xml mapper에서 사용한 id는 사용불가능하다. --%>
			<form name="NsmNews" action="/boardNotice/buView.do" method="post">
				<c:choose>
					<c:when test="${fn:length(selectListBuFrontList) > 0}">
						<c:forEach var="VselectListBuFrontList" items="${selectListBuFrontList}">
							...
							<tr>
								<!-- a태그로 처리요망 -->
								<td><a
									href="/boardNotice/buView.do?bUsers_id=${VselectListBuFrontList.bUsers_id}">${VselectListBuFrontList.buTitle}</a></td>
