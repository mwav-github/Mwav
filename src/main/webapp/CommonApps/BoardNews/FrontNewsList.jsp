				...
				<c:choose>
					<c:when test="${fn:length(selectListBnsFrontList) > 0}">
						<c:forEach var="VselectListBnsFrontList"
							...
							<!-- 날짜 확인//// -->
							<!-- 기사 업데이트 날짜 -->
							<c:set var="jstl_targetDate"
								value="${VselectListBnsFrontList.bnUpdateDt }" />
							<!-- 2000-01-01 00:00:00 -->
							<fmt:parseDate var="parse_targetDate" pattern="yyyy-MM-dd"
								value="${jstl_targetDate }" />
							<fmt:formatDate value="${parse_targetDate }" pattern="yyyy-MM-dd"
								var="targetdate" />
							...
							<c:set var="jstl_ago"
								value="<%=new java.util.Date(new java.util.Date().getTime() - (60 * 60 * 24 * 1000 * 21))%>" />
							<!-- 3주 -->
							<fmt:formatDate value="${jstl_ago}" pattern="yyyy-MM-dd"
								var="ago" />

							<fmt:parseDate var="parseTargetDate" pattern="yyyy-MM-dd"
								value="${targetdate }" />
							<fmt:parseDate var="parseAgoDate" pattern="yyyy-MM-dd"
								value="${ago }" />
							<fmt:parseNumber var="parseTdate"
								value="${parseTargetDate.time / (1000*60*60*24) }"
								integerOnly="true" />
							<fmt:parseNumber var="parseAgo"
								value="${parseAgoDate.time / (1000*60*60*24) }"
								integerOnly="true" />
							<!-- //// -->
							...
							<c:if test="${VselectListBnsFrontList.bnStatus eq '2'}">
								...
								<tr>
									<!-- a태그로 처리요망 -->
									<td class="col-md-12 text-left "><c:if
										...
										href="/board/bnsView.mwav?bNews_id=${VselectListBnsFrontList.bNews_id}#news_top">
											...
											<c:choose>
												<c:when
													test="${fn:length(VselectListBnsFrontList.bnTitle) < 70}">
													${VselectListBnsFrontList.bnTitle}
												</c:when>
												<c:otherwise>
													${fn:substring(VselectListBnsFrontList.bnTitle, 0, 70)} ...
												</c:otherwise>
											</c:choose> <%-- ${VselectListBnsFrontList.bnTitle} --%>
									</a></td>
						...
						<c:forEach var="VselectListBnsFrontList"
							...
							<c:if test="${VselectListBnsFrontList.bnStatus eq '2'}">
								...
								<tr class="center-block">
									<td class="col-xs-12  text-left "><c:if
											test="${parseTdate - parseAgo > 0}">
											<span class="label label-danger">new</span>
										</c:if><a
										href="/board/bnsView.mwav?bNews_id=${VselectListBnsFrontList.bNews_id}">
											...
											<c:choose>
												<c:when
													test="${fn:length(VselectListBnsFrontList.bnTitle) < 40}">
												${VselectListBnsFrontList.bnTitle}
											</c:when>
												<c:otherwise>
